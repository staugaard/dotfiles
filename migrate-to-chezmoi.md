...
- Numbered setup directories such as `00-core`, `10-zsh`, and `20-git`.
...
- `20-git/setup.rb` renders `.gitconfig` from `20-git/gitconfig.erb`.
...
- `dot_gitconfig.tmpl` renders `.gitconfig`.
- User-specific values come from chezmoi data.
- First-run prompting is handled by chezmoi config/template functions where appropriate.

Tasks:

- Convert `20-git/gitconfig.erb` to `dot_gitconfig.tmpl`.
- Define required data keys, likely:
  - `git.name`
  - `git.email`
  - any signing, GitHub, or work-specific values used by the template
- Add an initial `chezmoi.toml.tmpl` only if prompting/config bootstrapping is needed.
- Keep `20-git/setup.rb` until `dot_gitconfig.tmpl` has been applied successfully.
